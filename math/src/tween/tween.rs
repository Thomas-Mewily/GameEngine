...
pub struct Tween<T : Mul<float,Output=T> + Add<T,Output=T> + Clone + Copy + PartialEq + Debug>
{
    src  : T,
    dest : T,
...
impl<T : Mul<float,Output=T> + Add<T,Output=T> + Clone + Copy + PartialEq + Debug> Copy for Tween<T>{}
impl<T : Mul<float,Output=T> + Add<T,Output=T> + Clone + Copy + PartialEq + Debug> Clone for Tween<T>{
    fn clone(&self) -> Self {
...
impl<T : Mul<float,Output=T> + Add<T,Output=T> + Clone + Copy + PartialEq + Debug> Tween<T>
{
    pub fn new(val : T) -> Self { Self::new_animated(val.clone(), val, zero(), zero(), ___())}
